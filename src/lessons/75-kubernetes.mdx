...
<details>
  ...
  # Add your user to the k3s group so you can run kubectl without sudo
  sudo usermod -aG k3s $USER

  # Create a kubeconfig in your home directory
  mkdir -p "$HOME/.kube"
  sudo cp /etc/rancher/k3s/k3s.yaml "$HOME/.kube/config"
  sudo chown "$USER:$USER" "$HOME/.kube/config"

  # Make kubectl use the local config
  ...
  Notes:
  - If `kubectl` says “permission denied”, log out and back in (group membership refresh).
